...
import { host, adminAuth } from '@/config/constant';

const Icon = FoundationSymbol;
const { Row } = Grid;
const FormItem = Form.Item;

function AdminUserLogin(props) {
  ...
  async function fetchAdminData(params) {
    let result = [];
    console.log('登录请求：', `${host}${adminAuth.api.login}`, params);
    try {
      const data = await request({
        url: `${host}${adminAuth.api.login}`,
        method: 'POST',
        data: params
      });
      result = data.data;
  ...
  return (
    <div className={styles.userLogin}>
      ...
      <p className={styles.desc}>请先登录管理后台</p>
